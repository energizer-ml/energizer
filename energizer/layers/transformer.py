...
class MultiheadAttention(Module):
    def __init__(self, embed_dim: int, num_heads: int, dropout: float = 0.1, bias: bool = True, device: str = 'cpu'):
    ...
    def _reshape_for_attention(self, x: Tensor) -> Tensor:
        ...
        if self.device == 'gpu':
            x_reshaped = mx.reshape(x.data, (batch_size, seq_len, self.num_heads, self.head_dim))
            x_reshaped = mx.transpose(x_reshaped, (0, 2, 1, 3))
        else:
            x_reshaped = x.data.reshape(batch_size, seq_len, self.num_heads, self.head_dim)
            x_reshaped = x_reshaped.transpose(0, 2, 1, 3)

        return Tensor(x_reshaped, requires_grad=x.requires_grad, device=x.device)

    def _scaled_dot_product_attention(self, q: Tensor, k: Tensor, v: Tensor, attn_mask: Tensor = None, key_padding_mask: Tensor = None) -> Tensor:
        if self.device == 'gpu':
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, attn_mask: Tensor = None, key_padding_mask: Tensor = None) -> Tensor:
        q = self.q_proj(query)
        k = self.k_proj(key)
        v = self.v_proj(value)
...
class TransformerEncoderLayer(Module):
    def __init__(self, d_model: int, nhead: int, dim_feedforward: int = 2048, dropout: float = 0.1, activation: Union[str, Callable, Tensor] = "relu", layer_norm_eps: float = 1e-5, batch_first: bool = False, norm_first: bool = False, bias: bool = True, device: str = 'cpu'): 
        super().__init__(device=device)
        ...
        self.dropout_p = dropout
        self.self_attn = MultiheadAttention(d_model, nhead, dropout=dropout, bias=bias, device=device)
            
        self.linear1 = energizer.Linear(d_model, dim_feedforward, bias=bias, device=device)
        self.linear2 = energizer.Linear(dim_feedforward, d_model, bias=bias, device=device)
...
class TransformerEncoder(Module):
    def __init__(self, encoder_layer: TransformerEncoderLayer, num_layers: int, norm: Module = None, device: str = "cpu"):
        super().__init__(device=device)
