...
def fmt_energy(mj: float) -> str:
    if mj >= 1_000_000: return f"{mj/1_000_000:.2f}kJ"
    if mj >= 1_000:     return f"{mj/1_000:.2f}J "
    return f"{mj:.0f}mJ"
...
def render(state: State) -> str:
    ...
    status   = (C_DIM + f"  ⏱ {elapsed}   " + R +
                C_DIM + f"window \033[1m{state.hist_window}s\033[0m   " + R +
                C_DIM + f"interval \033[1m{state.interval}ms\033[0m   " + R +
                live)
    add(rpad(status, W))
    rule("─")
    ...
    add(rpad(
        C_WHITE + "\033[1m  Σ TOTAL" + R +
        C_WARN  + "\033[1m" + f"  {t_now:>5} mW" + R +
        C_DIM   + f"  pk {t_peak} mW" + R +
        C_DIM   + f"  avg {t_avg:.0f} mW" + R +
        C_DIM   + f"  session Σ {fmt_energy(t_mj)}" + R,
        W
    ))
    rule("─")
    ...
    # ── Keybinds ──────────────────────────────────────────────────────────────
    def badge(k, d):
        return bg(40,45,58) + C_WHITE + "\033[1m" + f" {k} " + R + C_DIM + f" {d}  " + R

    kb = "  "
    for k, d in [("q","quit"),("p","pause"),("r","reset pk"),
                 ("h","history"),("+ -","interval")]:
        kb += badge(k, d)
    add(kb)
    add("")
